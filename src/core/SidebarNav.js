  ...
  navDropdown(item, key) {
    ...
    return (
      <li key={key} className={this.activeRoute(item.url, this.props)}>
        <a className="nav-link nav-dropdown-toggle" href="#" onClick={(e) => this.handleClick(item.id, e)}><i className={classIcon} />{item.name}{this.navBadge(item.badge)}</a>
      ...
      <NavItem key={key} className={classes.item}>
        { attributes.disabled ?
            <RsNavLink href={""} className={classes.link} {...attributes}>
              {itemIcon}{item.name} {item.id} {itemBadge}
            </RsNavLink>
         :
          this.isExternal(url) ?
            ...
            <NavLink to={{ pathname: url, state: {id: item.id, name:item.name} }} className={classes.link} activeClassName="active" onClick={this.hideMobile} {...attributes}>
              {itemIcon}{item.name}{itemBadge}
            </NavLink>
        }
      </NavItem>
